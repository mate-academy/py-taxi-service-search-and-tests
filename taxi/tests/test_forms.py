...
class LicenseValidationTests(TestCase):
    ...
    def test_len_license_number(self):
        response = self.client.post(DRIVER_UPDATE_URL,
                                    data={"license_number": "TST1234"})

        self.assertFormError(response, "form", "license_number",
                             "License number should consist of 8 characters")

    def test_first_three_characters_uppercase(self):
        response = self.client.post(DRIVER_UPDATE_URL,
                                    data={"license_number": "tst12345"})
...
class SearchFormsTests(TestCase):
    ...
    def test_cars_search_porsche_911(self):
        response = self.client.get(
            reverse("taxi:car-list") + "?model=Porsche 911"
        )
    ...
    def test_manufacturers_search_porsche(self):
        response = self.client.get(
            reverse("taxi:manufacturer-list") + "?name=Porsche"
        )
