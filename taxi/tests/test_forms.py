...
from taxi.forms import DriverCreationForm, DriverSearchForm, ManufacturerSearchForm, CarSearchForm, \
    validate_license_number, LicenseForm
...
class FormsTests(TestCase):
    ...
    def test_driver_search_form_by_username(self):
        Driver.objects.create_user(username="test1", password="Test12345", license_number="RER12345")
        Driver.objects.create_user(username="test2", password="Test12342", license_number="RER12342")
        response = self.client.get(reverse("taxi:driver-list"), {"username": "test1"})
    ...
    def test_manufacturer_search_form_by_name(self):
        ...
        response = self.client.get(reverse("taxi:manufacturer-list"), {"name": "FAW Group"})
        manufacturers = Manufacturer.objects.filter(name__icontains="FAW Group")
        print("RESPONSE: ", response)
    ...
    def test_car_search_form_by_model(self):
        ...
        response = self.client.get(reverse("taxi:car-list"), {"model": "CHARD"})
