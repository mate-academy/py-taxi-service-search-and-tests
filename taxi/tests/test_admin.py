...
class AdminPanelTests(TestCase):
    def setUp(self) -> None:
        ...
        self.driver = get_user_model().objects.create_user(
            username="driver", password="driver123", license_number="ABC12345"
        )

    def test_driver_license_number_listed(self) -> None:
        """
        Test that driver's licence field is present in display_list
        for admin panel.
        return: None
        """
    ...
    def test_add_driver_page_contains_first_name_and_last_name_fields(
        self,
    ) -> None:
        """
        Test that the first name and last name fields are present
        on the add driver page in the admin panel.
        return: None
        """
        url = reverse("admin:taxi_driver_add")
        response = self.client.get(url)
